...
import { BoardMembersDataColumn } from '../../../../utils/datatablesource'
...
import Updatbordermember from './Updatbordermember';
...
const Boardmembers = () => {
    ...
    const handleShowUpdatePopup = (row) => {
        ...
        sessionStorage.setItem("updateboardMember", JSON.stringify(row));
    };
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            text: `${t('You will not be able to recover this')} ${t('Board Members')}!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    const isDeleted = await newRequest.delete("/deleteboard_members/" + row?.id);
                    if (isDeleted) {
                        toast.success(`${t('Board Members')}  ${t('has been deleted')} ${t('successfully')}!`, {
                            position: "top-right",
                            autoClose: 2000,
                    ...
                    toast.error(`${t('Board Members')} ${t('has been not deleted')} ${t('Delete')}!`, {
                        position: "top-right",
                        autoClose: 2000,
                        hideProgressBar: false,
                        closeOnClick: true,
    ...
    const handleAddCompany = async () => {
        ...
        try {
            ...
            toast.success(`Board Members ${job_title} "${name}" has been added successfully.`, {
                position: "top-right",
                autoClose: 2000,
                hideProgressBar: false,
                closeOnClick: true,
                ...
                json.forEach((item) => {
                    ...
                    newRequest.post(`/creatboard_members`, formData)
                        .then((res) => {
                            ...
                            Swal.fire(
                                'Add!',
                                `Board Members has been created`,
                                'success'
                            )
                            refreshcitiesData()
                        })
                        .catch((err) => {
                            ...
                            Swal.fire(
                                'Error!',
                                `Some Board Members already exist`,
                                'error'
                            )
                            // Handle errors
                        });
        ...
        <div>
            <div className={`p-0 h-full ${i18n.language === 'ar' ? 'sm:mr-72' : 'sm:ml-72'}`}>
                    ...
                    <div className="h-auto w-[97%] px-0 pt-4">
                        <div className="h-auto w-full p-0 bg-white shadow-xl rounded-md">
                            ...
                            <div style={{ marginLeft: '-11px', marginRight: '-11px' }}>
                                ...
                                <DataTable data={data}
                                    title={`${t('Board Members')}`}
                                    columnsName={BoardMembersDataColumn(t)}
                                    loading={isLoading}
                ...
                {isCreatePopupVisible && (
                    <Addboardmembers isVisible={isCreatePopupVisible} setVisibility={setCreatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
                {isUpdatePopupVisible && (
                    <Updatbordermember isVisible={isUpdatePopupVisible} setVisibility={setUpdatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
            </div>
        </div>
    )
