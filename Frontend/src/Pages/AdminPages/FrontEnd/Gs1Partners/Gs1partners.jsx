...
import { GS1PartnersDataColumn } from '../../../../utils/datatablesource'
...
import Addgs1partners from './Addgs1partners';
import Updatags1partners from './Updatags1partners';
...
const Gs1partners = () => {
    ...
    const handleShowUpdatePopup = (row) => {
        ...
        sessionStorage.setItem("updatePartners", JSON.stringify(row));
    };
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            text: `${t('You will not be able to recover this')} ${t('GS1 Partners')}!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    const isDeleted = await newRequest.delete("/deletepartners/" + row?.id);
                    if (isDeleted) {
                        toast.success(`${t('GS1 Partners')}  ${t('has been deleted')} ${t('successfully')}!`, {
                            position: "top-right",
                            autoClose: 2000,
                    ...
                    toast.error(`${t('GS1 Partners')} ${t('has been not deleted')} ${t('Delete')}!`, {
                        position: "top-right",
                        autoClose: 2000,
                        hideProgressBar: false,
                        closeOnClick: true,
    ...
    const handleAddCompany = async () => {
        ...
        const { imageshow, Page } = formValues;
        ...
        try {
            ...
            const response = await newRequest.post('/creatpartners/', formData);
        ...
        <div>
            <div className={`p-0 h-full ${i18n.language === 'ar' ? 'sm:mr-72' : 'sm:ml-72'}`}>
                ...
                {isCreatePopupVisible && (
                    <Addgs1partners isVisible={isCreatePopupVisible} setVisibility={setCreatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
                {isUpdatePopupVisible && (
                    <Updatags1partners isVisible={isUpdatePopupVisible} setVisibility={setUpdatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
            </div>
        </div>
    )
