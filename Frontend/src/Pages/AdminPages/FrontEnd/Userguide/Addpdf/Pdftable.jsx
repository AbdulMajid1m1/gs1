...
import { UserGuidepdfDataColumn } from '../../../../../utils/datatablesource'
...
import Adduserguidepdf from './Adduserguidepdf';
import Updtauserguidespdf from './Updtauserguidespdf';
...
const Pdftable = () => {
    ...
    const handleShowUpdatePopup = (row) => {
        ...
        sessionStorage.setItem("updateUserGuidepdf", JSON.stringify(row));
    };
    ...
    const handleDelete = async (row) => {
        Swal.fire({
            ...
            text: `${t('You will not be able to recover this')} ${t('User Guide pdf')}!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    const isDeleted = await newRequest.delete("/deleteuser_guide_pdfs/" + row?.id);
                    if (isDeleted) {
                        toast.success(`${t('User Guide pdf')}  ${t('has been deleted')} ${t('successfully')}!`, {
                            position: "top-right",
                            autoClose: 2000,
                    ...
                    toast.error(`${t('User Guide pdf')} ${t('has been not deleted')} ${t('Delete')}!`, {
                        position: "top-right",
                        autoClose: 2000,
                        hideProgressBar: false,
                        closeOnClick: true,
    ...
    const handleAddCompany = async () => {
        ...
        try {
            ...
            toast.success(`User Guide pdf ${job_title} "${name}" has been added successfully.`, {
                position: "top-right",
                autoClose: 2000,
                hideProgressBar: false,
                closeOnClick: true,
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                json.forEach((item) => {
                    ...
                    formData.append('job_title', item.job_title);
                    formData.append('description', item.Description);
                    formData.append('image', item.imageshow);
                    formData.append('addedBy', item.Page);
    ...
    return (
        <div>
            <div className='flex justify-start sm:justify-between items-center flex-wrap gap-2 py-7 px-3'>
                <CSVLink data={data}
                ...
                <button
                    ...
                    <i className="fas fa-plus mr-2"></i>{t('Add Pdf')}
                </button>
            ...
            <div style={{ marginLeft: '-11px', marginRight: '-11px' }}>
                ...
                {isCreatePopupVisible && (
                    <Adduserguidepdf isVisible={isCreatePopupVisible} setVisibility={setCreatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
                {isUpdatePopupVisible && (
                    <Updtauserguidespdf isVisible={isUpdatePopupVisible} setVisibility={setUpdatePopupVisibility} refreshBrandData={refreshcitiesData} />
                )}
            </div>
        </div>
    )
