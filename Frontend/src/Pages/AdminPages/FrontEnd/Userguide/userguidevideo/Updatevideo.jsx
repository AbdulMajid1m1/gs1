...
const Updatevideo = ({ isVisible, setVisibility, refreshBrandData }) => {
    ...
    const [selectedVideo, setSelectedVideo] = useState(updateBrandData?.video || '');
    ...
    const handleUpdateBrand = async () => {
        ...
        try {
            const response = await newRequest.put(`/updateuser_guide_videos/${updateBrandData?.id}`, formData);

            toast.success(response?.data?.message || `${t('Video File')} ${t('has been')} ${t('Updated Successfully')}.`, {
                position: "top-right",
                autoClose: 5000,
                    ...
                    <div className="popup-container h-auto sm:w-[45%] w-full">
                        <div className="popup-form w-full" style={{ maxHeight: '100vh', overflowY: 'auto' }}>
                            <form className='w-full'>
                                <h2 className='text-secondary font-sans font-semibold text-2xl'>{t('Edit')} {t('Video File')}</h2>
