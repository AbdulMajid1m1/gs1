...
const AdminActivityReport = () => {
  ...
  const fetchData = async (startDate, endDate) => {
    try {
      ...
      const res = await newRequest.post('/report/allgs1Admin', {
        startDate: formattedStartDate.toISOString(),
        endDate: formattedEndDate.toISOString(),
      });
  ...
  // UseEffect for default date setting and initial fetch
  useEffect(() => {
    const today = new Date();
    const oneMonthAgo = new Date(today.getFullYear(), today.getMonth() - 1, today.getDate());

    setStartDateChart(oneMonthAgo.toISOString().split('T')[0]);
    setEndDateChart(today.toISOString().split('T')[0]);

    fetchData(oneMonthAgo, today);
  }, []);

  // Existing handleAllAdminsSearchTimeAndDate function updated to use fetchData
  const handleAllAdminsSearchTimeAndDate = () => {
    if (!startDateChart || !endDateChart) {
      toast.info('Please select both start and end dates');
      return;
    }
    fetchData(startDateChart, endDateChart);
  };
          ...
          <div className="h-auto w-[97%] px-0 pt-4">
            <div className="h-auto w-full p-0 bg-white shadow-xl rounded-md">
              ...
              <div className='flex justify-between flex-wrap px-5'>
                <button
                  onClick={handleAllAdminsSearchTimeAndDate}
                  className="rounded-full bg-secondary font-body px-5 py-1 text-sm mb-3 text-white transition duration-200 hover:bg-primary"
                >
                  <i className="fas fa-eye ml-1"></i> {t('View All Admins Activity')}
                </button>
              </div>


              <BarLineChartJs data={chartData} />
