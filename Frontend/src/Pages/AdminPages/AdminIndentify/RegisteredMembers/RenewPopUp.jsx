...
const RenewPopUp = ({ isVisible, setVisibility }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      {
        toast.success(
          res?.data?.message || `${t("Renew request sent successfully!")}`
        );
        setLoading(false);
        // Close the popup
              ...
              <form onSubmit={handleSubmit} className="w-full">
                ...
                <div className="table-member-inoive px-4">
                  ...
                  <div className="flex justify-between items-center">
                    <h2 className="text-secondary font-sans text-sm">
                      {" "}
                      {t("Transaction ID")}:{" "}
                      {gs1RegesteredMembersData?.transaction_id}
                    </h2>
                    <div>
                      <h2 className="text-secondary font-sans text-sm">
                        {" "}
                        {t("GCP Expiry")}: {formattedExpiryDate}
                      </h2>
                      <h2 className="text-secondary font-sans text-sm">
                        {" "}
                        {t("New GCP")}: {formattedNewExpiryDate}
                      </h2>
                    </div>
                  </div>
                  <table>
