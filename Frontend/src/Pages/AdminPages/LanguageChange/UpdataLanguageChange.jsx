...
const UpdataLanguageChange = ({ isVisible, setVisibility, refreshBrandData }) => {
    ...
    const handleUpdateBrand = async () => {
        ...
        try {
            const response = await axios.put(backendUrl + `/translations/${updateBrandData?.LanguageID}`, {
                    ...
                    <div className="popup-container h-auto sm:w-[45%] w-full">
                        <div className="popup-form w-full">
                            <form className='w-full'>
                                <h2 className='text-secondary font-sans font-semibold text-2xl'>{t('Edit')}</h2>
                                <div className="flex flex-col sm:gap-3 gap-3 mt-5">
                                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-2">
                                        <label htmlFor="field1" className="text-secondary">{t('Word[English]')}</label>
                                        <input
                                            type="text"
                                            id="category_name_en"
                                    ...
                                    <div className="w-full font-body sm:text-base text-sm flex flex-col gap-2">
                                        <label htmlFor="field1" className="text-secondary">{t('Word [Arabic]')}</label>
                                        <input
                                            type="text"
                                            id="category_name_ar"
                                            value={category_name_ar}
