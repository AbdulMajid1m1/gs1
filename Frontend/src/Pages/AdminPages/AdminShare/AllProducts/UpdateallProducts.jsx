...
import DashboardRightHeader from "../../../../components/AdminDashboardRightHeader/AdminDashboardRightHeader";
...
const UpdateAllProducts = () => {
  ...
  const [gcp_type, setGcp_type] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        setGcp_type(productData?.gcp_type);
  ...
  const handleBrandNamesEnglishArabic = async () => {
    try {
      const response = await newRequest.get(`/brands?user_id=${user_id}`);
      // console.log(response.data);
      const data = response.data;
      const name = data.map((country) => country.name);
  ...
  const handleFormSubmit = async (e) => {
    ...
    formData.append("ProvGLN", barcode);
    ...
    formData.append("gcp_type", gcp_type || "GCP");
    ...
    formData.append("memberID", user_id);
  ...
  const computedEnglishValue = `${productNameEnglish  || ""} - ${size  || ""} - ${
    selectedBrandNameEnglish || ""
  }`;
  const computedArabicValue = `${productNameArabic  || ""} - ${size  || ""} - ${
    selectedBrandNameArabic || ""
  }`;
  ...
  const handleHsCodeAutoCompleteInputChange = async (
    event,
    newInputValue,
    reason
  ) => {
    ...
    try {
      ...
      const res = await newRequest.get(
        `/hsCode/searchHsCodes?term=${newInputValue}`,
        {
          signal: abortControllerRef.current.signal,
        }
      );
  ...
  return (
    <>
      ...
      >
        <div>
          <DashboardRightHeader title={`${t("Update Product")}`} />
        </div>
