...
export const createMemberDocument = async (req, res, next) => {
  ...
  try {
    ...
    TODO: await createMemberLogs(logData);
...
export const getMemberInvoices = async (req, res, next) => {
  try {
    ...
    if (error) {
      throw createError(
        400,
        `Invalid query parameter: ${error.details[0].message}`
      );
    }
  ...
  try {
    ...
    if (value.status === "approved") {
      await prisma.$transaction(
        async (prisma) => {
          ...
          if (cart && cart.cart_items) {
            ...
            if (product) {
              ...
              await prisma.gtin_subcriptions.updateMany({
                ...
                data: {
                  ...
                  gtin_subscription_total_price:
                    product.gtin_yearly_subscription_fee,
                },
              });
              ...
              for (const product of products) {
                ...
                let subscriptionFee =
                  userUpdateResult.membership_category === "non_med_category"
              ...
              await prisma.gtin_products.update({
                ...
                data: {
                  gcp_start_range: (
                    parseInt(product.gcp_start_range) + 1
                  ).toString(),
                },
              });
              ...
              if (tblSysNo) {
                ...
                await prisma.tblSysNo.update({
                  ...
                  data: {
                    TblSysCtrNo: (
                      parseInt(tblSysNo.TblSysCtrNo) + 1
                    ).toString(),
                  },
                });
              }
          ...
          const Certificatepath = await convertEjsToPdf(
            path.join(__dirname, "..", "views", "pdf", "certificate.ejs"),
            CertificateData,
            pdfFilePath,
            true
          );
      ...
      const pdfFilename1 = `Receipt-${existingUser?.company_name_eng}-${
        currentDocument.transaction_id
      }-${new Date().toLocaleString().replace(/[/\\?%*:|"<>]/g, "-")}.pdf`;
      ...
      const Receiptpath = await convertEjsToPdf(
      ...
      await sendStatusUpdateEmail(
        existingUser.email,
        value.status,
        value.status === "approved" ? { pdfBuffer, pdfFilename } : null,
        { pdfBuffer2, pdfFilename1 }
      );
      ...
      TODO: console.log("logData", logData);
      ...
      if (value.migration === true) {
        ...
        // Fetch other products subscriptions based on user_id and isDeleted=false
        const otherProductsSubscriptions =
          await prisma.other_products_subcriptions.findMany({
            where: {
        ...
        if (
          otherProductsSubscriptions.length > 0 &&
          otherProductsSubscriptions[0].product.product_name ===
            "GLN (30 Locations)"
        ) {
    ...
    if (value.checkBankSlip) {
      for (const document of bankSlipDocuments) {
        const deletingDocumentPath = path.join(
          __dirname,
          "..",
          "public",
          "uploads",
    ...
    if (value.status === "approved") {
      ...
      return res.json({
        message:
          "Document status updated to pending and bank slip documents deleted",
      });
    }
  } catch (err) {
    console.log(err);
...
export const regenerateGcpCertificate = async (req, res, next) => {
  try {
    ...
    const deletingDocumentPath = path.join(
      ...
      "..",
      "public",
      currentDocument.document.replace(/\\/g, "/")
    );
    console.log("deletingDocumentPath");
    console.log(deletingDocumentPath);
    try {
...
const sendStatusUpdateEmail = async (
  ...
  switch (status) {
    case "approved":
      ...
      emailContent =
        "<p>Your account has been activated. You can now access all the features.</p>";
      break;
    case "rejected":
      subject = "Your Gs1Ksa member Account is Rejected";
      let rejectionMessage = rejectReason
