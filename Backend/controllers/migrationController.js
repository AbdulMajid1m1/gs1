  ...
  try {
    ...
    if (!latestMembership) {
      return res
        .status(404)
        .json({
          message: "No active membership history found for this member.",
        });
    }
...
export const migrateUser = async (req, res, next) => {
    ...
    await sendEmail({
      ...
      toEmail: "binmarproperty@gmail.com", // TODO: change it
...
function mapMemberToNewUser(member) {
  ...
  let newUser = {
    ...
    email: "binmarproperty@gmail.com",
    ...
    memberID: member.MemberID
      ? member.MemberID.toString().replace(/\s/g, "")
      : "",
...
export async function exportMembersToExcel(req, res) {
  try {
    ...
    const latestMemberships = memberIDs.reduce((acc, memberId) => {
      const memberHistories = allMemberships.filter(
        (m) => m.MemberID === memberId
      );
      const latestHistory = memberHistories.reduce((latest, current) => {
        return latest.MembershipYear > current.MembershipYear
          ? latest
          : current;
  ...
  try {
    ...
    records.forEach((record) => {
      ...
      monthlyData[month].Total =
        monthlyData[month].Renewal + monthlyData[month].New;
    });
...
export const getgs1NewDbYearlyReport = async (req, res) => {
  try {
    ...
    const otherProductsSubscriptions =
      await prisma.other_products_subscription_histories.findMany({
        where: {
          created_at: {
            gte: new Date(`${year}-01-01`),
            lt: new Date(`${year + 1}-01-01`),
