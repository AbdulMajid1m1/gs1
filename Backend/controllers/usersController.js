...
export const sendInvoiceToUser = async (req, res, next) => {
    try {
        ...
        if (status === 'approved') {
            ...
            const emailData = [
                {
                    toEmail: userUpdateResult.email,
                    subject: `Invoice Generated`,
                    htmlContent: `<h1>Invoice Generated</h1>
                ...
                },
                {
                    ...
                    htmlContent: selectedLanguage === 'en' ?
                        ...
                        `<h1>You Generated invoice for ${userUpdateResult.company_name_eng} </h1>
                        <p>Invoice: <strong>${pdfFilename}</strong></p>
                        ` :
                        `<h1>لقد قمت بإنشاء فاتورة لـ ${userUpdateResult.company_name_arabic} </h1>
            ...
            if (req?.admin?.adminId) {
                ...
                const adminLog = {
                    subject: `Registed Products Approval & Invoice Generation by ${req?.admin?.username}`,
                    admin_id: req.admin.adminId,
...
export const createUser = async (req, res, next) => {
    try {
        ...
        const transaction = await prisma.$transaction(async (prisma) => {
            ...
            // if cart has more than one item then add them to other_products_subcriptions table
